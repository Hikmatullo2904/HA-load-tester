...
import uz.hikmatullo.loadtesting.repository.NodeRepository;
...
public class NodeServiceImpl implements NodeService {
    ...
    public NodeServiceImpl(NodeRepository repository, GroupRepository groupRepository) {
    ...
    public NodeResponse connect(NodeConnectRequest request) {
        ...
        NodeInfo node = new NodeInfo(request.groupId(), request.host());
    ...
    public List<NodeResponse> getNodesByGroup(String groupId) {
        List<NodeInfo> nodes = repository.findWorkerByGroupId(groupId);
        if (nodes.isEmpty()) {
    ...
    private NodeResponse toResponse(NodeInfo node) {
        return new NodeResponse(node.getId(), node.getGroupId(), node.getHost(), node.getConnectedAt());
    }
}
